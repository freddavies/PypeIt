...
def illum_profile_spectral(rawimg, waveimg, slits, slit_illum_ref_idx=0, smooth_npix=None, polydeg=None,
    ...
    for slit_idx, slit_spat in enumerate(slits.spat_id):
        ...
        if np.all(np.logical_not(onslit_init)):
            log.error(f"Slit {slit_idx} has no wavelength solution. Cannot perform relative spectral "
                      f"sensitivity calculation. You can turn off the relative spectral sensitivity "
